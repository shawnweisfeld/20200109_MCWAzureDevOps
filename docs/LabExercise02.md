...
### Task 1: Create Azure DevOps Account, Organization and Project
...
2.  If you do not already have an account, select the **Start free** button.
    ...
    > Be sure that you are authenticated with the SAME account that you used to log in to Azure with.

    > When you create an account it will create a default personal organization for you, and it will start the new project wizard. If you already have an account open your personal organization, and the press the "New project" button to start the wizard.

3.  Choose a name for new your project. For the purposes of this scenario, we will use "TailspinToys". Choose **Private** in the Visibility section so that our project is only visible to those who we specifically grant access. Then, select **+ Create project**.
...
4.  Once the Project is created, click on the **Repos** menu option in the left-hand navigation.
...
5.  On the **Repos** page for the **TailspinToys** repository, locate the "or push an existing repository from command line" section. Click the Copy button to copy the contents of the panel and save them in notepad. We're going to use these commands in an upcoming step.
...
6.  Push the changes up to the Azure DevOps repository with the following command. You will be prompted for a password, use the personal access token you created a moment ago. 
